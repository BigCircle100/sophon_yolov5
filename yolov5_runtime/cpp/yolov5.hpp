...
public:
  int get_batch_size();
  int yolov5_detect(std::vector<cv::Mat> &dec_images,
...
private:
  int yolov5_preprocess(std::vector<cv::Mat> &dec_images, 
                              std::vector<bm_tensor_t> &input_tensors);
  int yolov5_inference(std::vector<bm_tensor_t> &input_tensors,
