              ...
              m_nms_thresh(nms_thresh)
{
  ...
  for (int i = 0; i < m_batch_size; i++){
    auto ret = bm_image_create(m_handle, m_net_h, m_net_w, FORMAT_RGB_PLANAR, DATA_TYPE_EXT_1N_BYTE, &m_resized_bmimgs[i], strides);
  }
...
int YOLOv5::yolov5_detect(std::vector<cv::Mat> &dec_images,
                          std::vector<YoloV5BoxVec> &boxes)
{
  ...
  yolov5_preprocess(dec_images, input_tensors);

  yolov5_inference(input_tensors, output_tensors);

  yolov5_postprocess(dec_images, output_tensors, boxes);

}

void YOLOv5::yolov5_draw(int classId, 
                        float conf, 
                        int left, 
                        int top, 
                        int right, 
...
// 1b or 4b
int YOLOv5::yolov5_preprocess(std::vector<cv::Mat> &dec_images, 
                              std::vector<bm_tensor_t> &input_tensors)
{
  ...
  for (int i = 0; i < dec_images.size(); i++){
    ...
    auto ret = bmcv_image_vpp_convert_padding(m_handle, 1, bmimg_aligned, &m_resized_bmimgs[i],
  ...
  bmcv_image_convert_to(m_handle, m_batch_size, m_converto_attr, m_resized_bmimgs.data(), converto_bmimgs.data());
...
int YOLOv5::yolov5_inference(std::vector<bm_tensor_t> &input_tensors,
                            std::vector<bm_tensor_t> &output_tensors)
{

  bmrt_launch_tensor(m_bmrt, 
                        m_net_names[0], 
                        input_tensors.data(), 
                        m_input_num, 
                        output_tensors.data(), 
